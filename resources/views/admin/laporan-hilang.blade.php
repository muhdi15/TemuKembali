...
@section('page-title', 'Manajemen Laporan Kehilangan')
...
<div class="container-fluid">
    ...
    <div class="card shadow-sm border-0 rounded-3">
        <div class="card-header bg-maroon text-white d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><i class="bi bi-exclamation-triangle me-2"></i>Daftar Laporan Kehilangan</h5>
        ...
        <div class="card-body">
            @if($laporan->isEmpty())
                <p class="text-center text-muted mb-0">Belum ada laporan kehilangan.</p>
            @else
                ...
                <table class="table table-striped align-middle">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ \Carbon\Carbon::parse($data->tanggal_hilang)->format('d M Y') }}</td>
                            <td>
                                <span class="badge 
                            ...
                            <td>
                                <button class="btn btn-sm btn-outline-maroon" data-bs-toggle="modal" data-bs-target="#editModal{{ $data->id_laporan_hilang }}"><i class="bi bi-pencil-square"></i></button>
                                <form action="{{ route('admin.laporan-hilang.delete', $data->id_laporan_hilang) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                        ...
                        <!-- Modal Edit -->
                        <div class="modal fade" id="editModal{{ $data->id_laporan_hilang }}" tabindex="-1">
                            <div class="modal-dialog modal-lg">
                                <div class="modal-content border-0 shadow-sm">
                                    ...
                                    <form action="{{ route('admin.laporan-hilang.update', $data->id_laporan_hilang) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="text" name="lokasi_hilang" value="{{ $data->lokasi_hilang }}" class="form-control" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label>Tanggal Hilang</label>
                                                        <input type="date" name="tanggal_hilang" value="{{ $data->tanggal_hilang }}" class="form-control" required>
                                                    </div>
                                                    <div class="mb-3">
...
<div class="modal fade" id="modalTambah" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content border-0 shadow-sm">
            <div class="modal-header bg-maroon text-white">
                <h5 class="modal-title"><i class="bi bi-plus-circle"></i> Tambah Laporan</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('admin.laporan-hilang.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
